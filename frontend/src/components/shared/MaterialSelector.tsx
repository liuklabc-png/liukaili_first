...
export const MaterialSelector: React.FC<MaterialSelectorProps> = ({
  ...
  const loadMaterials = async () => {
    ...
    try {
      // 处理生成选项
      if (filterProjectId.startsWith('generate_')) {
        // 对于生成选项，我们不加载素材，而是显示空状态
        // 这样用户可以看到"暂无素材"的提示，然后可以点击生成按钮
        setMaterials([]);
        return;
      }
          ...
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div className="flex items-center gap-2 text-sm text-gray-600">
              ...
              {selectedMaterials.size > 0 && (
                <span className="ml-2 text-zollent-600">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  // 处理生成选项
                  if (value.startsWith('generate_')) {
                    // 保持展开状态，这样用户可以看到所有生成选项
                    setFilterProjectId(value);
                    return;
                  }
                  setFilterProjectId(value);
                }}
                className="px-3 py-1.5 text-sm border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-zollent-500 w-40 sm:w-48 max-w-[200px] truncate"
              >
                {/* 固定显示的三个选项 */}
                ...
                {showAllProjects && (
                  <>
                    ...
                    <option value="generate_ppt_green">生成一份关于绿色的ppt</option>
                    <option value="generate_ppt_tech">生成一份关于科技的ppt</option>
                    <option value="generate_ppt_business">生成一份关于商业的ppt</option>
                  </>
                )}
              </select>
            ...
            {materials.map((material) => {
              ...
              return (
                ...
                >
                  ...
                  {isSelected && (
                    <div className="absolute inset-0 bg-zollent-500 bg-opacity-20 flex items-center justify-center">
              ...
              <div className="mb-3 p-3 bg-gray-50 rounded-lg border border-gray-200">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    className="w-4 h-4 text-zollent-500 border-gray-300 rounded focus:ring-zollent-500"
                  />
