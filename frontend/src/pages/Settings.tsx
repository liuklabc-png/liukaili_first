...
// 模型套系配置
export type ModelSuite = 'nanozollent' | 'qwen' | 'doubao';

export const MODEL_SUITE_OPTIONS: { value: ModelSuite; label: string; description: string }[] = [
  { value: 'nanozollent', label: 'NanoZollent (OpenRouter)', description: '使用 Google Gemini 模型，通过 OpenRouter 代理' },
  { value: 'qwen', label: '千问 (Qwen)', description: '使用阿里千问系列模型' },
  { value: 'doubao', label: '豆包 (Doubao)', description: '使用火山引擎豆包系列模型' },
];

// 初始表单数据
const initialFormData = {
  model_suite: 'nanozollent' as ModelSuite,
  ai_provider_format: 'openai' as 'openai' | 'lazyllm',
  api_base_url: '',
  api_key: '',
  text_model: '',
  text_model_source: '',
    ...
    fields: [
      {
        ...
        label: '选择模型套系',
        type: 'buttons',
        description: '选择不同的模型套系将自动配置对应的模型参数。保存设置后生效。',
        options: [
          { value: 'nanozollent', label: 'NanoZollent' },
          { value: 'qwen', label: '千问' },
    ...
    fields: [
      {
        ...
        options: [
          ...
          { value: 'lazyllm', label: 'LazyLLM 格式' },
        ],
      },
      {
        key: 'api_base_url',
...
export const Settings: React.FC = () => {
  ...
  const loadSettings = async () => {
    ...
    try {
      ...
      if (response.data) {
        ...
        setFormData({
          model_suite: response.data.model_suite || 'nanozollent',
          ai_provider_format: response.data.ai_provider_format || 'openai',
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      const { api_key, mineru_token, baidu_ocr_api_key, model_suite, text_model_source, image_model_source, image_caption_model_source, ...otherData } = formData;
  ...
  const handleFieldChange = (key: string, value: any) => {
    // 禁止直接修改 ai_provider_format，只能通过选择模型套系来更改
    ...
    // 当选择模型套系时，自动更新相关配置
    if (key === 'model_suite') {
      const suite = value as ModelSuite;
      let newConfig: Partial<typeof initialFormData> = {};
      
      if (suite === 'nanozollent') {
        // NanoZollent 套系：使用 OpenRouter + Gemini
        newConfig = {
          ai_provider_format: 'openai',
          api_base_url: 'https://openrouter.ai/api/v1',
          text_model: 'google/gemini-2.0-flash-exp',
          text_model_source: '',
          image_model: 'google/gemini-3-pro-image-preview',
          image_model_source: '',
          image_caption_model: 'google/gemini-2.0-flash-exp',
          image_caption_model_source: '',
        };
      } else if (suite === 'qwen') {
        // 千问套系：使用 LazyLLM 格式
        newConfig = {
          ...
          image_caption_model: 'qwen-plus',
          image_caption_model_source: 'qwen',
        };
      } else if (suite === 'doubao') {
        // 豆包套系：使用 LazyLLM 格式
        newConfig = {
          ai_provider_format: 'lazyllm',
          api_base_url: '',
  ...
  const renderField = (field: FieldConfig) => {
    ...
    // 特殊处理 model_suite 字段 - 可以编辑
    if (field.key === 'model_suite' && field.type === 'buttons' && field.options) {
      return (
        <div key={field.key}>
          ...
          <div className="flex flex-wrap gap-2">
            {field.options.map((option) => (
              <button
                key={option.value}
                type="button"
                onClick={() => handleFieldChange('model_suite', option.value)}
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                  value === option.value
                    ? 'bg-gradient-to-r from-zollent-400 to-zollent-500 text-white shadow-md'
                    : 'bg-white border border-gray-200 text-gray-700 hover:border-zollent-300'
                }`}
              >
                {option.label}
              </button>
    ...
    if (field.type === 'buttons' && field.options) {
      // 处理AI提供商格式选项的禁用逻辑
      if (field.key === 'ai_provider_format') {
        return (
          <div key={field.key}>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              {field.label}
            ...
            <div className="flex flex-wrap gap-2">
              {field.options.map((option) => {
                // 根据当前模型套系决定是否禁用
                const isDisabled = (
                  (formData.model_suite === 'nanozollent' && option.value === 'lazyllm') ||
                  (formData.model_suite !== 'nanozollent' && option.value === 'openai')
                );
      ...
      return (
        <div key={field.key}>
          ...
          <select
            ...
            className="w-full h-10 px-4 rounded-lg border border-gray-200 bg-white focus:outline-none focus:ring-2 focus:ring-zollent-500 focus:border-transparent"
          >
            {field.options.map((option) => (
        ...
        <div key={field.key}>
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-zollent-500 focus:ring-offset-2 ${
                isEnabled ? 'bg-zollent-500' : 'bg-gray-200'
              }`}
            >
              <span
...
export const SettingsPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-zollent-50 to-yellow-50">
