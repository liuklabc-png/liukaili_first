...
ALLOWED_PROVIDER_FORMATS = {"openai", "lazyllm", "qwen"}
ALLOWED_MODEL_SUITES = {"nanozollent", "qwen", "doubao"}
...
def temporary_settings_override(settings_override: dict):
    ...
    try:
        # 应用覆盖设置
        if settings_override.get("model_suite"):
            original_values["MODEL_SUITE"] = current_app.config.get("MODEL_SUITE")
            current_app.config["MODEL_SUITE"] = settings_override["model_suite"]
...
def update_settings():
    ...
    try:
        ...
        # Update model suite configuration
        if "model_suite" in data:
            suite = data["model_suite"]
            if suite not in ALLOWED_MODEL_SUITES:
                allowed_values = "', '".join(sorted(ALLOWED_MODEL_SUITES))
                return bad_request(f"Model suite must be one of '{allowed_values}'")
            settings.model_suite = suite
            
            # Automatically set the corresponding provider format based on model suite
            if suite == "nanozollent":
                settings.ai_provider_format = "openai"
            elif suite == "qwen":
                settings.ai_provider_format = "qwen"
            else:  # doubao
                settings.ai_provider_format = "lazyllm"
            
            # Clear model settings when changing model suite to use backend defaults
        ...
        # Update AI provider format configuration (only if model suite is not provided)
        if "ai_provider_format" in data and "model_suite" not in data:
...
def _sync_settings_to_config(settings: Settings):
    ...
    logger.info(f"Syncing model_suite: {model_suite}")
    
    # Dynamically set AI_PROVIDER_FORMAT based on MODEL_SUITE
    if model_suite == 'nanozollent':
        current_app.config['AI_PROVIDER_FORMAT'] = 'openai'
        logger.info("Set AI_PROVIDER_FORMAT to openai for nanozollent")
    elif model_suite == 'qwen':
        current_app.config['AI_PROVIDER_FORMAT'] = 'qwen'
        logger.info("Set AI_PROVIDER_FORMAT to qwen for qwen")
    elif model_suite == 'doubao':
        current_app.config['AI_PROVIDER_FORMAT'] = 'lazyllm'
        logger.info("Set AI_PROVIDER_FORMAT to lazyllm for doubao")
        ...
        logger.info(f"Set AI_PROVIDER_FORMAT from settings: {settings.ai_provider_format}")
    else:
        # Default to openai
        current_app.config['AI_PROVIDER_FORMAT'] = 'openai'
        logger.info("Set AI_PROVIDER_FORMAT to openai by default")
    
    # Load model settings - use model suite specific defaults regardless of database settings
    if model_suite == 'nanozollent':
        current_app.config['TEXT_MODEL'] = 'google/gemini-3-flash-preview'
        current_app.config['IMAGE_MODEL'] = 'google/gemini-3-pro-image-preview'
        current_app.config['IMAGE_CAPTION_MODEL'] = 'google/gemini-3-flash-preview'
    elif model_suite == 'qwen':
        current_app.config['TEXT_MODEL'] = 'qwen-plus'
        current_app.config['IMAGE_MODEL'] = 'qwen-image-max'
        current_app.config['IMAGE_CAPTION_MODEL'] = 'qwen-plus'
    elif model_suite == 'doubao':
        current_app.config['TEXT_MODEL'] = 'qwen-plus'
        current_app.config['IMAGE_MODEL'] = 'doubao-seedream-4-0-250828'
        current_app.config['IMAGE_CAPTION_MODEL'] = 'qwen-plus'
    
    logger.info(f"Loaded TEXT_MODEL from model suite default: {current_app.config['TEXT_MODEL']}")
    logger.info(f"Loaded IMAGE_MODEL from model suite default: {current_app.config['IMAGE_MODEL']}")
    logger.info(f"Loaded IMAGE_CAPTION_MODEL from model suite default: {current_app.config['IMAGE_CAPTION_MODEL']}")
    ...
    # Sync MinerU settings (fall back to Config defaults if None)
    if settings.mineru_api_base is not None:
        if settings.mineru_api_base:
            ...
            # Clear override, fall back to Config
            if "MINERU_API_BASE" in current_app.config:
                current_app.config.pop("MINERU_API_BASE")
                logger.info("MINERU_API_BASE cleared, falling back to defaults")
    else:
        # Use Config default
        current_app.config["MINERU_API_BASE"] = Config.MINERU_API_BASE
        logger.info(f"Using MINERU_API_BASE from Config: {Config.MINERU_API_BASE}")
    
    # 总是同步 MINERU_TOKEN，确保从 Config 获取默认值
    if settings.mineru_token is not None:
        if settings.mineru_token:
            ...
            # Clear override, fall back to Config
            current_app.config["MINERU_TOKEN"] = Config.MINERU_TOKEN
            logger.info("MINERU_TOKEN cleared, using Config default")
    else:
        # Use Config default
        current_app.config["MINERU_TOKEN"] = Config.MINERU_TOKEN
        logger.info(f"Using MINERU_TOKEN from Config")
