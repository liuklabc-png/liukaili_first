...
class Config:
    ...
    # Model suite: nanozollent | qwen | doubao
    # This determines the default provider format and models
    MODEL_SUITE = os.getenv('MODEL_SUITE', 'nanozollent')
    
    # Provider format: gemini | openai | vertex | lazyllm | qwen
    # Dynamically set based on MODEL_SUITE
    if MODEL_SUITE == 'nanozollent':
        AI_PROVIDER_FORMAT = 'openai'
        TEXT_MODEL = 'google/gemini-3-flash-preview'
        IMAGE_MODEL = 'google/gemini-3-pro-image-preview'
        IMAGE_CAPTION_MODEL = 'google/gemini-3-flash-preview'
    ...
    # Qwen 格式专用配置（当 AI_PROVIDER_FORMAT=qwen 时使用）
    QWEN_API_KEY = os.getenv('QWEN_API_KEY', '')  # 当 AI_PROVIDER_FORMAT=qwen 时必须设置
    QWEN_API_BASE = os.getenv('QWEN_API_BASE', 'https://dashscope.aliyuncs.com/api/v1')
    QWEN_TIMEOUT = float(os.getenv('QWEN_TIMEOUT', '300.0'))  # 5分钟超时
    QWEN_MAX_RETRIES = int(os.getenv('QWEN_MAX_RETRIES', '2'))  # 最大重试次数
    ...
    # 并发配置
    MAX_DESCRIPTION_WORKERS = int(os.getenv('MAX_DESCRIPTION_WORKERS', '1'))
    MAX_IMAGE_WORKERS = int(os.getenv('MAX_IMAGE_WORKERS', '1'))
