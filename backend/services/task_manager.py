        ...
        try:
            ...
            def generate_single_image(page_id, page_data, page_index):
                ...
                with app.app_context():
                    ...
                    base_delay = 5  # 减少基础延迟时间（秒）
                    
                    # 根据模型类型设置不同的延迟策略
                    from services.ai_service_manager import get_ai_service
                    ai_service = get_ai_service()
                    image_model = ai_service.image_model
                    is_qwen_model = 'qwen' in image_model.lower()
                    ...
                    for attempt in range(max_retries):
                        try:
                            ...
                            if attempt > 0:
                                ...
                                logger.info(f"Rate limit exceeded! Retrying image generation for page {page_index} after {delay:.2f} seconds...")
                                time.sleep(delay)
                            else:
                                # 根据模型设置不同的初始延迟
                                if is_qwen_model:
                                    ...
                                    logger.info(f"Starting image generation for page {page_index} with {initial_delay:.2f}s delay (Qwen model)...")
                                    time.sleep(initial_delay)
                                else:
                                    # 其他模型（如豆包）不需要延迟
                                    initial_delay = random.uniform(0.5, 1.5)
                                    logger.info(f"Starting image generation for page {page_index} with {initial_delay:.2f}s delay (Other model)...")
                                    time.sleep(initial_delay)
                            
                            logger.info(f"Starting image generation for page {page_index}/{len(pages)}, attempt {attempt + 1}/{max_retries}")
                            ...
                            try:
                                ...
                                if not image:
                                    # 生成失败，但不是速率限制
                                    logger.error(f"Failed to generate image for page {page_id}: Image generation returned None")
                                    # 确保数据库会话状态正确
                                    try:
                                        db.session.rollback()
                                    except:
                                        pass
                                    return (page_id, None, "Image generation failed", None)
                                ...
                                if "429" in error_str or "Too Many Requests" in error_str or "rate limit" in error_str.lower():
                                    logger.warning(f"Rate limit exceeded for page {page_index}, attempt {attempt + 1}/{max_retries}")
                                    # 如果还有重试机会，继续循环
                                    if attempt < max_retries - 1:
                                        ...
                                        continue
                                    # 如果没有重试机会，返回错误
                                    logger.error(f"Failed to generate image for page {page_id}: Rate limit exceeded")
                                    # 确保数据库会话状态正确
                                    try:
                                        db.session.rollback()
                                    except:
                                        pass
                                    return (page_id, None, "Rate limit exceeded", None)
                                # 检查是否是400错误（参数错误）
                                elif "400" in error_str or "Bad Request" in error_str:
                                    logger.error(f"Bad request error for page {page_id}: {error_str}")
                                    # 400错误通常是参数问题，不需要重试
                                    try:
                                        db.session.rollback()
                                    except:
                                        pass
                                    return (page_id, None, f"Bad request: {error_str}", None)
                                # 其他错误
                                else:
                                    logger.error(f"Unexpected error for page {page_id}: {error_str}")
                                    try:
                                        db.session.rollback()
                                    except:
                                        pass
                                    return (page_id, None, f"Error: {error_str}", None)
                            
                            # Check resolution for all providers
                            actual_res, is_match = check_image_resolution(image, resolution)
                            if not is_match:
                                logger.debug(f"Resolution mismatch for page {page_index}: requested {resolution}, got {actual_res}")
                            ...
                            # 在成功生成图片后，根据模型类型添加不同的延迟
                            from services.ai_service_manager import get_ai_service
                            ai_service = get_ai_service()
                            image_model = ai_service.image_model
                            
                            if 'qwen' in image_model.lower():
                                # 千问模型需要延迟以避免速率限制
                                post_delay = random.uniform(8, 15)
                                logger.info(f"Adding {post_delay:.2f}s delay after successful image generation for page {page_index} (Qwen model)...")
                                time.sleep(post_delay)
                            else:
                                # 其他模型（如豆包）不需要延迟
                                post_delay = random.uniform(1, 3)
                                logger.info(f"Adding {post_delay:.2f}s delay after successful image generation for page {page_index} (Other model)...")
                                time.sleep(post_delay)
            ...
            # Use ThreadPoolExecutor for parallel generation
            # 关键：提前提取 page.id，不要传递 ORM 对象到子线程
            logger.info(f"Creating ThreadPoolExecutor with max_workers=1 (forced to avoid Qwen API rate limits)")
            # 强制使用max_workers=1，避免Qwen API速率限制
            with ThreadPoolExecutor(max_workers=1) as executor:
                ...
                for future in as_completed(futures):
                    ...
                    if task:
                        ...
                        # 移除分辨率不匹配的警告消息
                        if 'warning_message' in progress:
                            del progress['warning_message']
            ...
            if task:
                ...
                if resolution_mismatched > 0:
                    logger.debug(f"Task {task_id} has {resolution_mismatched} resolution mismatches")
                db.session.commit()
