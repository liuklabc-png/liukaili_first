...
# Cache keys are tuples (model, source) to handle different sources for the same model
...
def _get_cached_text_provider(model: str) -> TextProvider:
    ...
    """
    # Get text_model_source from Flask config
    source = None
    if has_app_context() and current_app and hasattr(current_app, "config"):
        source = current_app.config.get("TEXT_MODEL_SOURCE")
    
    # Create cache key based on both model and source
    cache_key = (model, source)
    
    with _cache_lock:
        if cache_key not in _text_provider_cache:
            logger.info(f"Creating new TextProvider for model: {model}, source: {source}")
            _text_provider_cache[cache_key] = get_text_provider(model=model)
        else:
            logger.debug(f"Reusing cached TextProvider for model: {model}, source: {source}")
...
def _get_cached_image_provider(model: str) -> ImageProvider:
    ...
    """
    # Get image_model_source from Flask config
    source = None
    if has_app_context() and current_app and hasattr(current_app, "config"):
        source = current_app.config.get("IMAGE_MODEL_SOURCE")
    ...
    with _cache_lock:
        if cache_key not in _image_provider_cache:
            logger.info(f"Creating new ImageProvider for model: {model}, source: {source}")
            _image_provider_cache[cache_key] = get_image_provider(model=model)
        else:
            logger.debug(f"Reusing cached ImageProvider for model: {model}, source: {source}")
...
def _get_cached_caption_provider(model: str) -> TextProvider:
    ...
    # Get image_caption_model_source from Flask config
    source = None
    if has_app_context() and current_app and hasattr(current_app, "config"):
        source = current_app.config.get("IMAGE_CAPTION_MODEL_SOURCE")
    ...
    with _cache_lock:
        if cache_key not in _caption_provider_cache:
            logger.info(f"Creating new CaptionProvider for model: {model}, source: {source}")
