...
LAZYLLM_VENDORS = {'qwen', 'doubao', 'glm', 'siliconflow', 'sensenova', 'minimax', 'kimi'}
...
def _build_provider_config() -> Dict[str, Any]:
    ...
    if fmt == 'openai':
        ...
        cfg['text_source'] = _resolve_setting('TEXT_MODEL_SOURCE', 'qwen')
        cfg['image_source'] = _resolve_setting('IMAGE_MODEL_SOURCE', 'qwen')
        logger.info("Provider config — format: lazyllm, text_source: %s, image_source: %s",
                     cfg['text_source'], cfg['image_source'])
                      
    elif fmt == 'qwen':
        cfg['api_key'] = _resolve_setting('QWEN_API_KEY')
        cfg['api_base'] = _resolve_setting('QWEN_API_BASE', 'https://dashscope.aliyuncs.com/api/v1')
        if not cfg['api_key']:
            raise ValueError(
                "QWEN_API_KEY (from database settings or environment) "
                "is required when AI_PROVIDER_FORMAT=qwen."
            )
        logger.info("Provider config — format: qwen, api_base: %s", cfg['api_base'])
...
def _get_model_type_provider_config(model_type: str) -> Dict[str, Any]:
    ...
    elif source_lower == 'qwen':
        api_key = _resolve_setting(f'{prefix}_API_KEY') or _resolve_setting('QWEN_API_KEY')
        api_base = _resolve_setting(f'{prefix}_API_BASE') or _resolve_setting('QWEN_API_BASE', 'https://dashscope.aliyuncs.com/api/v1')
        if not api_key:
            raise ValueError(
                f"API key is required for {model_type} model with Qwen provider. "
                f"Set {prefix}_API_KEY or QWEN_API_KEY."
            )
        logger.info("Per-model config — %s: qwen, api_base: %s", model_type, api_base)
        return {'format': 'qwen', 'api_key': api_key, 'api_base': api_base}
...
def get_caption_provider(model: str = "gemini-3-flash-preview") -> TextProvider:
    ...
    if 'qwen' in model.lower():
        # 对于qwen模型，直接使用QwenTextProvider
        from .text.qwen_provider import QwenTextProvider
        api_key = _resolve_setting('QWEN_API_KEY')
        api_base = _resolve_setting('QWEN_API_BASE', 'https://dashscope.aliyuncs.com/api/v1')
        if not api_key:
            raise ValueError(
                "QWEN_API_KEY (from database settings or environment) "
                "is required when using qwen model."
            )
        logger.info("Caption provider: Qwen, model=%s", model)
        return QwenTextProvider(api_key=api_key, api_base=api_base, model=model)
...
def get_text_provider(model: str = "gemini-3-flash-preview") -> TextProvider:
    ...
    if 'qwen' in model.lower():
        ...
        logger.info("Text provider: Qwen, model=%s", model)
        return QwenTextProvider(api_key=api_key, api_base=api_base, model=model)
    ...
    if fmt == 'openai':
        ...
        source = config.get('source') or config.get('text_source')
        # 如果source为空，根据模型名称推断提供商
        if not source:
            if 'doubao' in model.lower():
                source = 'doubao'
...
def get_image_provider(model: str = "gemini-3-pro-image-preview") -> ImageProvider:
    ...
    """
    # 首先检查模型名称是否包含'doubao'或'qwen'
    if 'doubao' in model.lower():
        # 对于doubao模型，使用LazyLLMImageProvider
        logger.info("Image provider: LazyLLM, model=%s, source=doubao", model)
        return LazyLLMImageProvider(source='doubao', model=model)
    elif 'qwen' in model.lower():
        # 对于qwen模型，直接使用QwenImageProvider
        from .image.qwen_provider import QwenImageProvider
        api_key = _resolve_setting('QWEN_API_KEY')
        api_base = _resolve_setting('QWEN_API_BASE', 'https://dashscope.aliyuncs.com/api/v1')
        if not api_key:
            raise ValueError(
    ...
    if fmt == 'openai':
        ...
        source = config.get('source') or config.get('image_source')
        # 如果source为空，根据模型名称推断提供商
        if not source:
            if 'doubao' in model.lower():
                source = 'doubao'
        ...
        from .image.qwen_provider import QwenImageProvider
        logger.info("Image provider: Qwen, model=%s", model)
        return QwenImageProvider(api_key=config['api_key'], api_base=config['api_base'], model=model)
