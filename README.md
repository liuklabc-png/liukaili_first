...
<img width="256" src="./frontend/public/logo.svg">
...
[![GitHub Stars](https://img.shields.io/github/stars/Anionex/zollent-slides?style=square&color=FFD700)](https://github.com/Anionex/zollent-slides/stargazers)
[![GitHub Forks](https://img.shields.io/github/forks/Anionex/zollent-slides?style=square&color=FFD700)](https://github.com/Anionex/zollent-slides/network)
[![GitHub Watchers](https://img.shields.io/github/watchers/Anionex/zollent-slides?style=square&color=FFD700)](https://github.com/Anionex/zollent-slides/watchers)


[![Version](https://img.shields.io/badge/version-v0.4.0-44cc11)](https://github.com/Anionex/zollent-slides)
![Docker](https://img.shields.io/badge/Docker-Build-4A90D9?logo=docker&logoColor=white)
[![License](https://img.shields.io/github/license/Anionex/zollent-slides?color=0055aa)](https://github.com/Anionex/zollent-slides/blob/main/LICENSE)
...
<b>Zollent Slides - 基于 AI 的智能 PPT 生成与编辑工具</b>

<b>🎯 降低PPT制作门槛，让每个人都能快速创作出美观专业的演示文稿</b>

<b>🚀 支持从想法/大纲/页面描述生成完整PPT，自动提取附件图表，支持自然语言修改，导出可编辑的PPTX文件</b>
...
传统的AI PPT生成工具虽然能快速生成PPT，但存在诸多限制：
...
Zollent Slides 应运而生，它基于先进的 AI 模型，能够生成高质量、风格统一的PPT页面，同时提供灵活的编辑能力，让你既能快速生成，又能自由调整，真正实现"Vibe PPT"的愿景。

### 🌟 模型多样性优势

不同于其他类似项目（如蕉幻），Zollent Slides 支持三套模型轮换，为用户提供更多选择，制作PPT更加随心：

- **第一套：Nanovabanana 模型**（原NanoBanana）：包含 Google Gemini 系列模型，生成质量高，风格多样
  - 文本模型：google/gemini-3-flash-preview
  - 图像模型：google/gemini-3-pro-image-preview
  - 图片识别模型：google/gemini-3-flash-preview
  - 使用 OpenRouter 代理，访问稳定

- **第二套：千问模型**：包含阿里千问系列模型，中文理解能力强，适合国内用户
  - 文本模型：qwen-plus
  - 图像模型：qwen-image-max
  - 图片识别模型：qwen-plus
  - 使用国内接口，无需代理

- **第三套：豆包模型**：包含火山引擎豆包系列模型，国内访问速度快，无需代理
  - 文本模型：qwen-plus
  - 图像模型：doubao-seedream-4-0-250828
  - 图片识别模型：qwen-plus
  - 使用国内接口，访问速度快

**国内用户福利**：千问和豆包两套模型均为国内模型，无需挂代理即可正常使用，访问速度快，稳定性高。用户可以根据自己的需求和网络环境选择合适的模型套系，制作PPT更加灵活随心。
...
更多可见<a href="https://github.com/Anionex/zollent-slides/issues/2" > 使用案例 </a>
...
- **大纲模式**：直接编辑大纲，AI 基于大纲生成对应内容
- **页面描述**：详细描述每一页的内容和布局，AI 精准生成
- **自然语言编辑**：支持以口语化方式修改内容（如"把第三页改成案例分析"）

<img width="2000" height="1125" alt="创作路径" src="https://github.com/user-attachments/assets/7fc1ecc6-433d-4157-b4ca-95fcebac66ba" />
...
<img width="1920" height="1080" alt="素材处理" src="https://github.com/user-attachments/assets/8cda1fd2-2369-4028-b310-ea6604183936" />

### 💬 自然语言修改
- **局部重绘**：对不满意的区域进行口头式修改（如"把这个图换成饼图"）
- **整页优化**：生成高清、风格统一的页面
- **智能调整**：AI 理解上下文，保持整体风格一致性

<img width="2000" height="1125" alt="自然语言修改" src="https://github.com/user-attachments/assets/929ba24a-996c-4f6d-9ec6-818be6b08ea3" />

### 📤 多种导出格式
- **PPTX 导出**：一键导出标准 PPTX 文件，支持后续编辑
- **PDF 导出**：直接导出为 PDF 格式，方便分享和打印
- **可编辑 PPTX**：导出高还原度、可自由编辑的 PPT 页面（Beta）

<img width="1000" alt="导出格式" src="https://github.com/user-attachments/assets/3e54bbba-88be-4f69-90a1-02e875c25420" />
...
### ✨ 可编辑 PPTX 导出（Beta）
- **高还原度**：导出的 PPT 页面保持原始设计风格
- **可自由编辑**：文字和图像都可以单独编辑
- **智能排版**：自动处理文字大小、颜色和位置

<img width="1000" alt="可编辑PPTX" src="https://github.com/user-attachments/assets/a85d2d48-1966-4800-a4bf-73d17f914062" />
...
- 【1-4】 : v0.4.0发布：可编辑pptx导出全面升级：
  ...
  * **详细效果和使用方法见 https://github.com/Anionex/zollent-slides/issues/121**
...
1. **克隆代码仓库**
```bash
git clone https://github.com/Anionex/zollent-slides
cd zollent-slides
```
...
编辑 `.env` 文件，配置必要的环境变量：
> **项目中大模型接口以 AIHubMix 平台格式为标准，推荐使用 [AIHubMix](https://aihubmix.com/?aff=17EC) 获取 API 密钥，减小迁移成本**<br>
> **友情提示：AI 模型接口可能产生费用，请合理使用**
```env
# 模型套系配置（可选：nanozollent | qwen | doubao）
# nanozollent: 使用 Google Gemini 模型，通过 OpenRouter 代理
# qwen: 使用阿里千问系列模型，国内访问无需代理
# doubao: 使用火山引擎豆包系列模型，国内访问无需代理
MODEL_SUITE=nanozollent
...
**使用新版可编辑导出配置方法，获得更好的可编辑导出效果**: 需在[百度智能云平台](https://console.bce.baidu.com/iam/#/iam/apikey/list)（点击此处进入）中获取API KEY，填写在.env文件中的BAIDU_API_KEY字段（有充足的免费使用额度）。详见https://github.com/Anionex/zollent-slides/issues/121 中的说明
...
镜像名称：
- `anoinex/zollent-slides-frontend:latest`
- `anoinex/zollent-slides-backend:latest`
...
```bash
# 查看后端日志（最后 200 行）
docker logs --tail 200 zollent-slides-backend

# 实时查看后端日志（最后 100 行）
docker logs -f --tail 100 zollent-slides-backend

# 查看前端日志（最后 100 行）
docker logs --tail 100 zollent-slides-frontend
```
...
**注：感谢优秀开发者朋友 [@ShellMonster](https://github.com/ShellMonster/) 提供了[新人部署教程](https://github.com/ShellMonster/zollent-slides/blob/docs-deploy-tutorial/docs/NEWBIE_DEPLOYMENT.md)，专为没有任何服务器部署经验的新手设计，可[点击链接](https://github.com/ShellMonster/zollent-slides/blob/docs-deploy-tutorial/docs/NEWBIE_DEPLOYMENT.md)查看。**
...
- （可选）[LibreOffice](https://www.libreoffice.org/) - 使用「PPT 翻新」功能上传 PPTX 文件时需要，用于将 PPTX 转换为 PDF。**推荐先在本地将 PPTX 转为 PDF 后再上传**，原因：LibreOffice 在服务端渲染时可能因缺少字体（如微软雅黑、Calibri 等）导致排版错位，且无法完整还原部分特效。上传 PDF 文件则不需要 LibreOffice。Docker 用户如仍需在容器内支持 PPTX 上传，可执行：
  ```bash
  docker exec -it zollent-slides-backend bash -c "apt-get update && apt-get install -y libreoffice-impress && rm -rf /var/lib/apt/lists/*"
  ```
...
2. **导出可编辑 ppt 效果不佳，如文字重叠、无样式等**
    - 90% 情况为 API 配置出现问题。可以参考 [issue 121](https://github.com/Anionex/zollent-slides/issues/121) 中的排查与解决方案。
...
欢迎通过
[Issue](https://github.com/Anionex/zollent-slides/issues)
和
[Pull Request](https://github.com/Anionex/zollent-slides/pulls)
为本项目贡献力量！
...
[![Contributors](https://contrib.rocks/image?repo=Anionex/zollent-slides)](https://github.com/Anionex/zollent-slides/graphs/contributors)
...
<a href="https://www.star-history.com/#Anionex/zollent-slides&type=Timeline&legend=top-left">

 <picture>

   <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=Anionex/zollent-slides&type=Timeline&theme=dark&legend=top-left" />

   <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/svg?repos=Anionex/zollent-slides&type=Timeline&legend=top-left" />

   <img alt="Star History Chart" src="https://api.star-history.com/svg?repos=Anionex/zollent-slides&type=Timeline&legend=top-left" />
